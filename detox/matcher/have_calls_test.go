...
var _ = DescribeFunction(matcher.HaveCalls, func() {
	...
	DescribeFunction(types.GomegaMatcher.FailureMessage, func() {
		...
		It("should return failure message properly formatted", func() {
			...
			Expect(observed).To(MatchRegexp("Expected Interface1\\.SingleArgNoReturn \\(.*/matcher/have_calls_test\\.go \\[18\\]\\) to have following calls:\\n\\t\\t\\[0\\]: \\[<string> second\\]\\n\\t\\t\\[1\\]: \\[<string> first\\]\\n\\t\\t\\[2\\]: \\[<string> third\\]\\n, but received calls were:\\n\\t\\t\\[0\\]: \\[<string> first\\]\\n\\t\\t\\[1\\]: \\[<string> second\\]"))
		})
	})
	...
	DescribeFunction(types.GomegaMatcher.NegatedFailureMessage, func() {
		...
		It("should return failure message properly formatted", func() {
			...
			Expect(observed).To(MatchRegexp("Expected Interface1\\.SingleArgNoReturn \\(.*/matcher/have_calls_test\\.go \\[18\\]\\) not to have following calls:\\n\\t\\t\\[0\\]: \\[<string> second\\]\\n\\t\\t\\[1\\]: \\[<string> first\\]\\n\\t\\t\\[2\\]: \\[<string> third\\]\\n, but received calls were:\\n\\t\\t\\[0\\]: \\[<string> first\\]\\n\\t\\t\\[1\\]: \\[<string> second\\]"))
		})
	})
})
