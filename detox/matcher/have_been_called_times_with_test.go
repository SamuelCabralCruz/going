...
var _ = DescribeFunction(matcher.HaveBeenCalledTimesWith, func() {
	...
	BeforeEach(func() {
		...
		cut = matcher.HaveBeenCalledTimesWith(3, inputs...)
	})
	...
	DescribeFunction(types.GomegaMatcher.FailureMessage, func() {
		...
		It("should return failure message properly formatted", func() {
			...
			Expect(observed).To(MatchRegexp("Expected Interface1\\.SingleArgNoReturn \\(.*/matcher/have_been_called_times_with_test\\.go \\[20\\]\\) to have been called 3 times with following args:\\n\\t\\t\\[<string> some input arg\\]\\n, but received calls were:\\n\\t\\t\\[0\\]: \\[<string> first\\]\\n\\t\\t\\[1\\]: \\[<string> second\\]"))
		})
	})
	...
	DescribeFunction(types.GomegaMatcher.NegatedFailureMessage, func() {
		...
		It("should return failure message properly formatted", func() {
			...
			Expect(observed).To(MatchRegexp("Expected Interface1\\.SingleArgNoReturn \\(.*/matcher/have_been_called_times_with_test\\.go \\[20\\]\\) not to have been called 3 times with following args:\\n\\t\\t\\[<string> some input arg\\]\\n, but received calls were:\\n\\t\\t\\[0\\]: \\[<string> first\\]\\n\\t\\t\\[1\\]: \\[<string> second\\]"))
		})
	})
})
